CREATE TABLE IF NOT EXISTS {schema_name}.{safety_msg_table_name} (
	...
	CONSTRAINT safety_message_robot_id_fkey FOREIGN KEY ({msg_foreign_key_column_name}) 
				REFERENCES {schema_name}.{robot_info_table_name}({robot_pk_column_name})
);
CREATE INDEX IF NOT EXISTS "MDSafetyMessage_robot_info_fk" ON {schema_name}.{safety_msg_table_name} USING btree ({msg_foreign_key_column_name});